...
- `api/creators.js` — hides `New`/`Enriching`/`Failed` from every view (keeps `Needs Review` for the Review tab); now also returns `Source`.
- `api/add-manual.js` — writes the handle straight into `Creators` as `Status=New, Source=Manual`, deduped against the one table (dropped the 3-table dedup loop and the Manual staging table).
- `api/intake.js` — off-list intake creates now stamp `Source=Inbound` (and `Status=Not Contacted`), so kit-link signups who weren't on the outreach list are flagged.
- `web/admin.html` — new **📋 Review** tab (only place `Needs Review` shows; **✓ Approve** sets `Not Contacted`); a 🔗 **Inbound** source badge on the handle cell; `Needs Review` hidden from all other tabs.
- `.claude/skills/scout-creators/SKILL.md` — rewritten for the single table: enrich `Status=New` rows in place, set `Needs Review` (AI) / `Not Contacted` (manual) / `Failed`; dedup within the one table.
- Retired `scripts/manual-to-confirmed.airtable.js` + `scripts/approved-to-confirmed.airtable.js`. **Airtable-UI follow-ups (not API-doable):** delete those 2 automations, delete the 2 staging tables, delete the leftover `Stage` field.
- Reference visual: `docs/Creator Pipeline - How It Works.excalidraw`.

## 2026-06-01 — Amazon affiliate tracking in Airtable (replaces Levanta at low volume)
- **Why** — Levanta (~$50/mo) isn't justifiable with only a handful of creators. We replicate its core — per-creator tracked links, commission tracking, payout reconciliation — manually with free Amazon-native tools (Amazon Attribution tags + unique promo codes) and the existing CRM. Revisit a paid tool past ~20–30 active creators.
- `scripts/setup-affiliate-tracking.airtable.py` — New idempotent, additive-only setup script (Metadata API). Added 4 per-creator fields to **Creators (confirmed)** (`tblbBNgHxp6YNOJOQ`): `Amazon Attribution Link` (url), `Founding Creator` (checkbox — first 50 = 25% permanent), `Payout Method` (PayPal/Wise/Venmo), `Payout Handle`. Reuses existing `Affiliate Code` (→ Amazon promo code, e.g. `STEPH20`) and `Commission Rate` (20% / 25%). The old WooCommerce-era fields (`Affiliate Link`, `Total Conversions`, `Total Revenue`) were left untouched.
- Airtable schema — Created **Affiliate Payouts** table (`tblHjmJP5kHddLqgG`): one row per creator per month. Fields: `Payout` (primary), `Creator` (link → confirmed), `Month` (2026-05…12), `Units Sold`, `Sales Revenue`, `Commission Rate`, `Commission Owed`, `Payout Status` (Pending/Paid), `Paid Date`, `Payout Reference`, `Notes`. You enter Units/Revenue from the Amazon Attribution dashboard each month; track what's owed + paid.
- **Manual UI follow-ups** (Airtable API can't create formula/rollup fields): set `Commission Owed` = `{Sales Revenue} * {Commission Rate}` on Affiliate Payouts; add rollups on Creators (confirmed) for Total Units / Total Sales / Total Commission Owed / Total Paid from the linked Affiliate Payouts; add a "Pending Payouts" view filtered to `Payout Status = Pending`.
